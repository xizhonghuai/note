...
def data_init():
    ...
    # 为了演示效果取部分图片
    x_train_np = np.array(x_train, dtype='float32')
    return np.reshape(x_train_np, newshape=(60000, 28, 28, 1))
...
def batch_normalization(value, in_size, training=True):
    ...
    # tf.compat.v1.layers.batch_normalization(value, training=training)
...
# 定义全连接层函数
def add_layer(input, in_size, out_size, active_function=None, isDropout=False):
    # input 输入矩阵
    # in_size 输入矩阵列大小
    # out_size 输出矩阵列大小
    # active_function 激活函数
    ...
    # 激励输入
    z_i = tf.matmul(input, weighs) + bais
    if isDropout is True:
        z_i = tf.nn.dropout(z_i, 0.2)
...
def generator_inference(input_tensor, reuse_variables=False):
    ...
    with tf.name_scope("G"):
        with tf.compat.v1.variable_scope("generator_1", reuse=reuse_variables):
            out_0 = add_layer(input_tensor, 100, 7 * 7 * 256, tf.nn.leaky_relu)
            out_1 = tf.reshape(out_0, (batch_size, 7, 7, 256))
        with tf.compat.v1.variable_scope("generator_2", reuse=reuse_variables):
            out_2 = add_cnn_transpose_layer(out_1
                                            , filter_shape=[5, 5, 128, 256]
                                            , strides_shape=[1, 1, 1, 1]
                                            , out_shape=[batch_size, 7, 7, 128]
                                            , active_function=tf.nn.leaky_relu)
            out_2 = batch_normalization(out_2, 128, training=not reuse_variables)
        ...
        with tf.compat.v1.variable_scope("generator_3", reuse=reuse_variables):
            out_3 = add_cnn_transpose_layer(out_2
                                            ...
                                            , active_function=tf.nn.leaky_relu)
            out_3 = batch_normalization(out_3, 64, training=not reuse_variables)


        with tf.compat.v1.variable_scope("generator_4", reuse=reuse_variables):
            out_4 = add_cnn_transpose_layer(out_3
...
def discriminator_inference(input_tensor, reuse_variables=False):
    ...
    with tf.name_scope("D"):
        ...
        with tf.compat.v1.variable_scope("discriminator_2", reuse=reuse_variables):
            # 输入(None,nodes)
            # 输出 (None,512)
            out_5 = add_layer(reshaped, nodes, 512, tf.nn.leaky_relu, isDropout=not reuse_variables)
...
# 高斯分布噪声
def create_gaussian_noise(batch_size):
    array = np.array(np.random.normal(loc=0.0, scale=0.2, size=(batch_size, 100)), dtype='float32')
    return array
...
def train():
    ...
    # 生成器输入（随机噪声）
    noise_x = tf.compat.v1.placeholder(tf.float32, [None, 100])
    # 真实图片的概率
    real_p = discriminator_inference(input_tensor=image_x)
    # 生成器生成的图片
    fake_images = generator_inference(input_tensor=noise_x)
    ...
    g_vars, d_vars = get_trainable_variables()
    # 使用tf.train.GradientDescentOptimizer 优化算法来优化损失函数。
    train_step_d = tf.compat.v1.train.AdamOptimizer(learning_rate=0.0004).minimize(loss=d_loss
                                                                                   ,
                                                                                   global_step=global_step_d
                                                                                   , var_list=d_vars)
    train_step_g = tf.compat.v1.train.AdamOptimizer(learning_rate=0.0004).minimize(loss=g_loss
                                                                                   , global_step=global_step_g
                                                                                   , var_list=g_vars)
    ...
    with tf.compat.v1.Session() as sess:
        ...
        for i in range(TRAINING_STEPS):
            ...
            if i % 4 == 0:
                ...
                fake_imgs = np_normalize(fake_imgs[0:8], 0, 1)
...
def train():
    ...
    # 损失函数 (去掉log)
    d_loss = -1. * tf.reduce_mean(real_p) + tf.reduce_mean(fake_p)
    g_loss = -1. * tf.reduce_mean(fake_p)


    # 获取需要训练的变量
    g_vars, d_vars = get_trainable_variables()
    # 使用tf.train.GradientDescentOptimizer 优化算法来优化损失函数。
    train_step_d = tf.compat.v1.train.RMSPropOptimizer(learning_rate=0.0004).minimize(loss=d_loss
                                                                                   ,
                                                                                   global_step=global_step_d
                                                                                   , var_list=d_vars)
    train_step_g = tf.compat.v1.train.RMSPropOptimizer(learning_rate=0.0004).minimize(loss=g_loss
                                                                                   , global_step=global_step_g
                                                                                   , var_list=g_vars)

    # 在更新判断判别器网络参数前，先限制网络参数
    clip_d = [var.assign(tf.clip_by_value(var, -0.05, 0.05)) for var in d_vars]
