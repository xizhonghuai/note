...
def read_data():
    # 数据集地址:https://github.com/susanli2016/Machine-Learning-with-Python/blob/master/diabetes.csv
    # data.txt 数据结构说明
    # Pregnancies/Glucose/BloodPressure/SkinThickness/Insulin/BMI/DiabetesPedigreeFunction/Age/Outcome
    # (怀孕次数，血糖，血压，皮脂厚度，胰岛素，BMI身体质量指数，糖尿病遗传函数，年龄，标记结构(0/1))
    # 共768个样本
    data = np.loadtxt("data.txt")
    data = np_normalize(data, 0, 1)
    # 打乱顺序
    ...
    # 训练样本个数
    train_specimen_number = 650
    train_data = data[0:train_specimen_number]
    test_data = data[train_specimen_number:]
    train_data_x_y = np.split(train_data, [8], axis=1)
    test_data_x_y = np.split(test_data, [8], axis=1)
...
def active_function(z_array):
    # 归一化后的标签 1->0.001182033096926714  0->0
    # 设置0.0005为一个阈值，大于等于则为1，否则为0
    return np.where(z_array >= 0.0005, 1, 0)
    # return z_array
...
def validate_model(w_params, data_feed):
    ...
    train_err = np.abs(train_y_ - active_function(train_y))
    train_err_rate = np.sum(train_err) / train_err.shape[0]
    print("train_err_rate = %.8f" % train_err_rate)

    # 计算泛化误差
    test_y_ = model(w_params, test_x)
    test_err = np.abs(test_y_ - active_function(test_y))
    test_err_rate = np.sum(test_err) / test_err.shape[0]
    print("test_err_rate = %.8f" % test_err_rate)
