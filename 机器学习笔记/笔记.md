...
$$
\begin{cases}
\frac{\partial L(x,α,β)}{\partial x}|_{x=x^*} = 0  \qquad(1) \\ 
α_i \not=0 \qquad (2)\\
β_i \ge 0 \qquad (3) \\
β_ig_i(x^*) =0 \qquad (4) \\
h_i(x^*)=0  \qquad (5)\\
...
\end{cases}
$$
如果$β \gt 0 $,则有：$x_1=5,x_2=-5,α=12,β=-16$。显然β不满足不等式约束情况。

如果$β = 0 $,则有：$x_1=1,x_2=-1,α=4$。

所以当$x_1=1,x_2=-1$时，函数f取得最小值：8

## 4.5 拉格朗日对偶问题

SVM的目标函数（硬间隔），带有复杂的约束条件，计算时比较困难。为了方便计算，可以将原来的优化问题（原问题）转换成另一个问题（对偶问题）进行优化求解。
设原问题的最优解为P，对偶问题的最优解为D，则满足：
$$
D \le P
$$
**对偶问题最优解是原问题最优解的下界。**
...
写成广义的拉格朗日函数为：
$$
L(x,α,β)=f(x)+\sum_{i=1}^kα_ih_i(x)+\sum_{i=1}^lβ_ig_i(x) \qquad( α_i \not=0; β_i \ge 0 )
$$
当$x$取值一定的情况下，讨论函数$L(x,α,β)$的最大值,令$θ_p(x)=\underset{α,β}{max} L(x,α,β)$：
$$
θ_p(x)=\underset{α,β}{max} L(x,α,β)=f(x)+\sum_{i=1}^kα_ih_i(x)+\sum_{i=1}^lβ_ig_i(x) \qquad( α_i \not=0; β_i \ge 0 )
$$
当$x$不满足原始约束问题时，即存在某个$h_i(x) \not=0$或某个$g_i(x)\gt0$。

那么，通过调整$α$可以使得$\sum_{i=1}^kα_ih_i(x)=+\infin$,同理调整$β$,可以使得$\sum_{i=1}^lβ_ig_i(x)=+\infin$。所以有：
$$
θ_p(x)= +\infin
$$
当$x$满足原始约束优化问题时，调整$α,β$,可以使得$\sum_{i=1}^kα_ih_i(x)+\sum_{i=1}^lβ_ig_i(x)=0$,所以有：
$$
θ_p(x)= f(x)
$$
即：
$$
θ_p(x)=\underset{α,β}{max} L(x,α,β)=\begin{cases}
 f(x)  \qquad(x满足原始约束条件)\\ 
 +\infin \qquad(x不满足原始约束条件)\\
\end{cases}
$$

当对$θ_p(x)$,求最小值时，可以发现和原问题是等价的。
$$
\underset{x}{min}θ_p(x)=\underset{x}{min} \underset{α,β}{max} L(x,α,β)=\underset{x}{min}f(x)  \qquad(f(x)中的x满足原始约束条件)
$$
至此，得到一个重要结论，原问题：
$$
\begin{cases}
...
等价于
$$
\underset{x}{min} \underset{α,β}{max} L(x,α,β) \qquad( α \not=0; β \ge0)
$$

**原问题带不等式约束条件的目标函数f(x)的优化等价于对应的拉格朗日函数极小极大优化**
定义：
$$
\underset{α,β}{max} \underset{x}{min} L(x,α,β) \qquad( α \not=0; β \ge0)
$$

令$p=\underset{x}{min} \underset{α,β}{max} L(x,α,β);d=\underset{α,β}{max} \underset{x}{min} L(x,α,β)$,下面将分析d与p的关系。

将$ \underset{x}{min} L(x,α,β)$中的x加入约束条件，则有：
$$
\underset{x}{min} L(x,α,β) \le \underset{x \in 可行域}{min} L(x,α,β)=\underset{x \in 可行域}{min} [f(x)+\sum_{i=1}^kα_ih_i(x)+\sum_{i=1}^lβ_ig_i(x)] \qquad( α_i \not=0; β_i \ge 0 )
$$
因为$x$加入约束条件则有$\sum_{i=1}^kα_ih_i(x)=0,\sum_{i=1}^lβ_ig_i(x) \le0$,那么f(x)加上一个非正数结果一定是小于等于f(x)。所以：
$$
\underset{x \in 可行域}{min} [f(x)+\sum_{i=1}^kα_ih_i(x)+\sum_{i=1}^lβ_ig_i(x)] \le\underset{x \in 可行域}{min} f(x)\qquad( α_i \not=0; β_i \ge 0 )
$$
那么可以推出：
$$
\underset{x}{min} L(x,α,β) \le \underset{x \in 可行域}{min} L(x,α,β) \le \underset{x \in 可行域}{min} f(x)\qquad( α_i \not=0; β_i \ge 0 )
$$
=>
$$
\underset{α,β}{max}\underset{x}{min} L(x,α,β) \le  \underset{α,β}{max}\underset{x \in 可行域}{min} L(x,α,β) \le \underset{x \in 可行域}{min} f(x)\qquad( α_i \not=0; β_i \ge 0 )
$$
> $\underset{x \in 可行域}{min} f(x) 与α,β已经没有关系，所以可以去掉\underset{α,β}{max}$

=>
$$
\underset{α,β}{max}\underset{x}{min} L(x,α,β) \le \underset{x \in 可行域}{min} f(x)\qquad( α_i \not=0; β_i \ge 0 )
$$
=>
$$
d \le p
$$
**拉格朗日函数的极大极小问题的最优解是极小极大问题最优解的下界。称$\underset{α,β}{max} \underset{x}{min} L(x,α,β)$是$\underset{x}{min} \underset{α,β}{max} L(x,α,β)$的对偶问题**。
**若原问题是凸优化，那么$x^*α^*,β^*$分别是原问题与对偶问题的最优解（d=p），且$x^*α^*,β^*$满足KKT条件**。
...
写出拉格朗日函数：
$$
L(\omega,b,β)=\frac{1}{2}||\pmb{\omega}||^2+\sum_{i=1}^nβ_i[1-y_i(\pmb{\omega}^T\pmb{x_i}  + b)] \qquad(β_i \ge 0,x_i \in R^d,n表示样本数量，d表示x_i的维度 )\\
=\frac{1}{2}\omega^T\omega+\sum_{i=1}^nβ_i - \sum_{i=1}^nβ_iy_i(\pmb{\omega}^T\pmb{x_i}  + b) \\
=\frac{1}{2}(\omega_1^2+\omega_2^2+...+\omega_d^2)+(β_1+β_2+...+β_n)-[β_1y_1(\pmb{\omega}^T\pmb{x_1}  + b)+β_2y_2(\pmb{\omega}^T\pmb{x_2}  + b)+...+β_ny_n(\pmb{\omega}^T\pmb{x_n}  + b)]
$$
对$\omega$求导：
$$
\frac{\partial L(\omega,b,β)}{\partial \omega} = \omega-(β_1y_1x_1+β_2y_2x_2+...+β_ny_nx_n)=0
$$
=>
$$
\omega = (β_1y_1x_1+β_2y_2x_2+...+β_ny_nx_n)=\sum_{i=1}^nβ_iy_ix_i
$$
$\frac{1}{2}(\omega_1^2+\omega_2^2+...+\omega_d^2)$是一个标量，令其为S，则标量S对向量$\omega$求导后，会得到一个向量，这个向量就是$[\frac{\partial S}{\partial \omega_1},\frac{\partial S}{\partial \omega_2},,,\frac{\partial S}{\partial \omega_d}]=[\omega_1,\omega_2,,,\omega_d]=\omega$

对$b$求导：
$$
\frac{\partial L(\omega,b,β)}{\partial b}=β_1y_1+β_2y_2+...+β_ny_n=0
$$
=>
$$
\sum_{i=1}^nβ_iy_i=0
...
将$\omega =\sum_{i=1}^nβ_iy_ix_i;\sum_{i=1}^nβ_iy_i=0带入L(\omega,b,β)$入后得：
$$
L(\omega,b,β)=\frac{1}{2}\omega^T\omega+\sum_{i=1}^nβ_i - \sum_{i=1}^nβ_iy_i(\pmb{\omega}^T\pmb{x_i}  + b)\\
=\frac{1}{2}(\sum_{j=1}^nβ_jy_jx_j)^T(\sum_{i=1}^nβ_iy_ix_i)+\sum_{i=1}^nβ_i-\sum_{i=1}^nβ_iy_i\omega^Tx_i-\sum_{i=1}^nβ_iy_ib\\
=\frac{1}{2}(\sum_{j=1}^nβ_jy_jx_j)^T(\sum_{i=1}^nβ_iy_ix_i)+\sum_{i=1}^nβ_i-\sum_{i=1}^nβ_iy_i(\sum_{j=1}^nβ_jy_jx_j)^Tx_i-b\sum_{i=1}^nβ_iy_i\\
=\frac{1}{2}(\sum_{j=1}^nβ_jy_jx_j)^T(\sum_{i=1}^nβ_iy_ix_i)+\sum_{i=1}^nβ_i-\sum_{i=1}^n\sum_{j=1}^nβ_iy_iβ_jy_jx_j^Tx_i\\
=\frac{1}{2}\sum_{i=1}^n\sum_{j=1}^nβ_iy_iβ_jy_jx_j^Tx_i+\sum_{i=1}^nβ_i-\sum_{i=1}^n\sum_{j=1}^nβ_iy_iβ_jy_jx_j^Tx_i\\
=\sum_{i=1}^nβ_i-\frac{1}{2}\sum_{i=1}^n\sum_{j=1}^nβ_iy_iβ_jy_jx_j^Tx_i
$$

构造函数$L(\omega,b,β)$的极大极小问题，则得到SVM的对偶问题：
$$
\underset{β}{max} \underset{\omega,b}{min} L(\omega,b,β)
=>\\
\underset{β}{max} \qquad \sum_{i=1}^nβ_i-\frac{1}{2}\sum_{i=1}^n\sum_{j=1}^nβ_iy_iβ_jy_jx_j^Tx_i
$$
>$$st .\\ 
>β\ge 0;\\
>\sum_{i=1}^nβ_iy_i=0\\
...
解出$β$后，可得到线性可分SVM的决策函数：
$$
f(x)=\pmb{\omega}^T\pmb{x}  + b\\
=(\sum_{i=1}^nβ_iy_ix_i)^Tx+b
$$
最优超平面为:
$$
\pmb{\omega}^T\pmb{x}  + b
=(\sum_{i=1}^nβ_iy_ix_i)^Tx+b=0
$$
...
图中是无法找到一个超平面完全将样本正确划分。因此软间隔概念的引入即是为了解决线性不可分问题。

软间隔的思想是允许部分样本分类错误，但又希望总的错误尽量小。则优化问题变为：
$$
\begin{cases}
\underset{\omega,b}{min}  \qquad \frac{1}{2}||\pmb{\omega}||^2 + c \sum_{i=1}^n ξ_i \qquad  (ξ_i \ge 0)\\
y_i(\pmb{\omega}^T\pmb{x_i}  + b) \ge 1-ξ_i \qquad 
\end{cases}
$$

> c是一个常数，可以理解成表示的是一个误差项的权重，$ξ_i$表示样本$x_i$不满足强硬约束条件的程度,称为松弛变量, n表示样本个数。

软间隔将之前的强硬条件$y_i(\pmb{\omega}^T\pmb{x_i}  + b) \ge 1$ ，放宽为$y_i(\pmb{\omega}^T\pmb{x_i}  + b) \ge 1-ξ_i$ ,即允许了不部分样本点不满足之前的强硬条件。

对约束条件变形得：
$$
\begin{cases}
\underset{\omega,b}{min}  \qquad \frac{1}{2}||\pmb{\omega}||^2 + c \sum_{i=1}^n ξ_i  \\
1-ξ_i-y_i(\pmb{\omega}^T\pmb{x_i}  + b) \le 0 \\
-ξ_i \le 0
\end{cases}
$$
...
写出拉格朗日函数为：
$$
L(\omega,b,ξ,β,u) = \frac{1}{2}||\pmb{\omega}||^2 + c \sum_{i=1}^n ξ_i +\sum_{i=1}^nβ_i(1-ξ_i-y_i(\pmb{\omega}^T\pmb{x_i}  + b) )-\sum_{i=1}^nu_iξ_i  \qquad(β,u \ge 0)
$$
令$L(\omega,b,ξ,β,u) 对\omega,b,ξ$分别求导后得：
$$
\omega =\sum_{i=1}^nβ_iy_ix_i \\
\sum_{i=1}^nβ_iy_i=0 \\
c=β_i+u_i
$$
将上式代入$L(\omega,b,ξ,β,u)$后：
$$
L(\omega,b,ξ,β,u)=\sum_{i=1}^nβ_i-\frac{1}{2}\sum_{i=1}^n\sum_{j=1}^nβ_iy_iβ_jy_jx_j^Tx_i 
$$
对偶问题为：
$$
\underset{β}{max} \underset{\omega,b}{min}L(\omega,b,ξ,β,u)
=>\\
\underset{β}{max} \qquad \sum_{i=1}^nβ_i-\frac{1}{2}\sum_{i=1}^n\sum_{j=1}^nβ_iy_iβ_jy_jx_j^Tx_i
$$
>$$st .\\ 
...
**加入松弛变量后的优化模型与没有加入的形式上一样，唯一区别是多了条件$β\le c$,c的选取根据实验效果而定**

## 4.8 核函数
很多实际问题中，输入样本空间的数据存在完全不能线性可分的情况（异或问题），即找不到一个超平面将原始样本空间正确划分。即使引入软间隔方式也不可能得到很高的准确率。为了解决这个问题，其思想是将原始样本空间的数据进行维度提升。
根据定理，原始样本每升高一个维度，其线性可分得概率将会增加。所以将维度升高一定程度后，就一定能找一个超平面将数据完全划分。
设原始样本空间数据为$x$,经过一个变换函数$φ(.)$得到一个高维特征空间数据$φ(x)$。则对于数据$φ(x)$,只要能找到一个合适的函数$φ(.)$,就能够找到一个超平面将数据$φ(x)$完全划分正确。

总结一下就是，将原始样本空间通过特殊变换（这个变换又叫特征映射函数），得到高维的数据，然后在高维数据中仍然使用线性可分的SVM模型执行分类。这样就可以实现对低纬空间中非线性可分数据进行分类处理。

![12](img\12.png)

将函数$φ(.)$带入SVM优化目标函数得：
$$
\underset{β}{max} \qquad \sum_{i=1}^nβ_i-\frac{1}{2}\sum_{i=1}^n\sum_{j=1}^nβ_iy_iβ_jy_jφ(x_j)^Tφ(x_i)
$$
>$$
>st .\\ 
>β\ge 0;\\
...
在优化以上目标函数时，实际上是很困难的，首先要找到一个合适的变换函数，找到后需执行$φ(x_j)^Tφ(x_i)$（为特征空间数据的内积形式）运算。在实际应用中，特征空间数据的维度可能比较高（甚至是无穷维的），在执行内积运算时，开销很大。
因此为了解决寻找变换函数与计算问题引入核函数。
若存在一个函数k，使得：
$$
k(x_j,x_i)=φ(x_j)^Tφ(x_i)
$$
称函数k为核函数，引入核函数后，并不需要关心$φ$具体的形式是什么，将来的内积运算也转换成了其他运算。
将k函数带入优化目标函数后得到：
$$
\underset{β}{max} \qquad \sum_{i=1}^nβ_i-\frac{1}{2}\sum_{i=1}^n\sum_{j=1}^nβ_iy_iβ_jy_jk(x_j,x_i)
$$
>$$
>st .\\ 
>β\ge 0;\\
...
解出$β$后，得到决策函数：
$$
f(x)=\pmb{\omega}^T\pmb{φ(x)}  + b\\
=(\sum_{i=1}^nβ_iy_iφ(x_i))^Tφ(x)+b \\
=\sum_{i=1}^nβ_iy_iφ(x_i)^Tφ(x)+b \\
=\sum_{i=1}^nβ_iy_ik(x_i,x)+b
$$
最优超平面为:
$$
\sum_{i=1}^nβ_iy_ik(x_i,x)+b=0
$$

综上所述，原始输入空间经过特征映射函数变换后，在特征空间内线性可分。特征映射函数的选择直接影响了SVM模型的好坏，而特征映射函数的功能隐式的由核函数完成，不合适的核函数将会得到一个不好的特征空间，所以核函数的选择非常重要。常用的核函数如下：
![13](img\13.png)

一般应用中需要通过调试去尝试各种核函数。选取效果最好的。
