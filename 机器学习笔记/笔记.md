...
下，最大化间隔d:
$$
max(\frac{2c}{||\pmb{\omega}||})
$$

由于c是常数，替换成1变形得：
$$
\begin{cases}
max  \qquad \frac{2}{||\pmb{\omega}||} \qquad (优化函数)\\
...
$$
\begin{cases}
min  \qquad ||\pmb{\omega}||^2 + k \sum_{i=1}^n e_i \qquad  (e_i \ge 0)\\
y_i(\pmb{\omega}^T\pmb{x_i}  + b) \ge 1-e_i \qquad 
\end{cases}
$$

> k是一个常数，表示的是一个误差项的权重，$e_i$表示样本$x_i$不满足强硬约束条件的程度, n表示样本个数。
...
$$
min  \qquad ||\pmb{\omega}||^2 + k \sum_{i=1}^n max(0,1- y_i(\pmb{\omega}^T\pmb{x_i}  + b))
$$
