...
最易选择可理解成在作决策的时候纠结程度小的（或**混乱程度、不确定性程度**，容易选择的混乱程度小，不容易选择的混乱程度大），首先选择容易选择的特征作为决策节点，根据决策结果，导出新的问题，然后在重新选择一个最容易选择的节点作为决策节点（子节点）。
...
public class DecisionTree {
    ...
    public void train(final List<TrainData> trainDataSet, final List<String> featureNames) {
        Node node = new Node();
        if (rootNode == null) {
            rootNode = node;
        }
        //如果数据集中标签全部一样，则直接标记为叶节点，并设置标签值
        //或者trainDataSet中特征的条件熵一致则为叶节点，标签值为类别最多的
        if (isSameEntropy(trainDataSet, featureNames) || isSameClass(trainDataSet)) {
        ...
        node.addDecisionFunction(o -> Objects.equals(o, node.getFeatureValue()));
        ...
        for (Object featureObject : featureObjects) {
            ...
            if (subsetTrainDataSet == null || subsetTrainDataSet.isEmpty()) {
                ...
                //不是空集，则递归
                node.setFeatureValue(featureObject);
                train(subsetTrainDataSet, featureNames);
            }
        }
    }
}
