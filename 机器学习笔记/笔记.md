...
$$
\pmb{\omega} =\pmb A \pmb x^T \pmb y
$$
最终多元线性回归模型可表示为：
$$
f(\pmb{ \hat x_i}) = \pmb{\hat x_i} \pmb{\omega}= \pmb{\hat x_i}(\pmb A \pmb x^T \pmb y) \quad (\pmb{\hat x_i}\in R^{d+1} ，\pmb{\hat x_i}=\{1,x_{i1},x_{i2},x_{i3},...,x_{id}\})
$$
